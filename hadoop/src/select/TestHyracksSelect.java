...
public class TestHyracksSelect {
	...
	public static void main(String[] args) {
		...
		//[A query can have multiple roots - not the case here. I still kept a list structure]
	    ...
	    //2. create the logican plan based on the roots above, to pass to createCompiler
	    ...
	    try {
			...
			e1.printStackTrace();
		} //this calls does the rewrites of the rules on the plan, which rules you want to execute here??
		...
		//umbrella for all the runtime properties are stored in jobGenContext later passed to PlanCompiler
		JobGenContext jobGenContext = new JobGenContext(outerFlowSchema, metaData, appContext, serializerDeserializerProvider, hashFunctionFactoryProvider, hashFunctionFamilyProvider, comparatorFactoryProvider, typeTraitProvider, booleanInspectorFactory, integerInspectorFactory, printerFactoryProvider, nullWriterFactory, normalizedKeyComputerFactoryProvider, expressionRuntimeProvider, expressionTypeComputer, nullableTypeComputer, typingContext, expressionEvalSizeComputer, partialAggregationTypeComputer, frameSize, clusterLocations);
		...
		//Simulate Hyracks planning to create IoperatorNodePushable, once I have these
		
	    IHyracksJobBuilder jobBuilder = new JobBuilder(spec, context.getClusterLocations());
        for (Mutable<ILogicalOperator> opRef : plan.getRoots()) {
            ...
            roots.add((Mutable<ILogicalOperator>) opRef.getValue()); //why is a cast mandatory here??
        }
