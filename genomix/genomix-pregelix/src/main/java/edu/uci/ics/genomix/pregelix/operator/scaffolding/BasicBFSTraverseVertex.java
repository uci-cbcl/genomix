...
public class BasicBFSTraverseVertex
	...
	public void addFakeVertex(String fakeKmer) {
		synchronized (lock) {
			...
			if (!fakeVertexExist) {
				// add a fake vertex
				Vertex vertex = (Vertex) BspUtils.createVertex(getContext()
						.getConfiguration());
	...
	public int updateBFSLength(BFSTraverseMessage incomingMsg,
		...
		case SRC_OFFSET:
			...
			readHeadSet = srcIsFlip ? vertex.getEndReads() : vertex
					.getStartReads();
		...
		case DEST_OFFSET:
			...
			readHeadSet = destIsFlip ? vertex.getEndReads() : vertex
					.getStartReads();
			offset = readHeadSet.getOffsetFromReadId(incomingMsg.getReadId());
			return destIsFlip ? totalBFSLength + offset - kmerSize + 1
					: totalBFSLength + internalKmerLength - offset - kmerSize
							+ 1;
		case WHOLE_LENGTH:
			return totalBFSLength + internalKmerLength - kmerSize + 1;
		default:
			throw new IllegalStateException(
					"Update length type only has two kinds: offset and whole_length!");
		}
	...
	public VKmer setSrcAndOutgoingMsgForDest(long readId,
		...
		READHEAD_ORIENTATION firstReadHeadType = searchInfoList.get(0)
				.getReadHeadOrientation();
		VKmer secondKmer = searchInfoList.get(1).getSrcOrDestKmer();
		READHEAD_ORIENTATION secondReadHeadType = searchInfoList.get(1)
				.getReadHeadOrientation();
		VKmer srcNode;
		if (firstIsSrc) {
			srcNode = firstKmer;
	...
	public void sendMsgToPathNodeToAddCommondReadId(
			HashMapWritable<LongWritable, PathAndEdgeTypeList> pathMap) {
		VertexValueWritable vertex = getVertexValue();
		for (LongWritable commonReadId : pathMap.keySet()) {
			...
			EDGETYPE prevToMe = edgeTypeList
					.get(pathAndEdgeTypeList.size() - 2);
			VKmer preKmer = kmerList
					.getPosition(pathAndEdgeTypeList.size() - 2);

			vertex.getEdgeMap(prevToMe.mirror()).get(preKmer)
					.add(commonReadId.get()); // Option: put 2 for test
		}
	}
	...
	public void appendCommonReadId(Iterator<BFSTraverseMessage> msgIterator) {
		...
		while (msgIterator.hasNext()) {
			...
			if (pathList.size() > 2)
				throw new IllegalStateException(
						"When path node receives message to append common readId,"
								+ "PathList should only have one(next) or two(prev and next) elements!");
			for (int i = 0; i < pathList.size(); i++) {
				vertex.getEdgeMap(edgeTypeList.get(i))
						.get(pathList.getPosition(i)).add(commonReadId);
			}
		}
	}
	...
	public boolean isValidOrientation(BFSTraverseMessage incomingMsg) {
		...
		default:
			throw new IllegalStateException(
					"ReadHeadType only has two kinds: FLIPPED AND UNFLIPPED!");
		}
	}
