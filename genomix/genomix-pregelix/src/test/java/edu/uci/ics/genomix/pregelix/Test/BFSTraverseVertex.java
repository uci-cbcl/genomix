...
public class BFSTraverseVertex extends BasicBFSTraverseVertex {
	...
	public void initVertex() {
		...
		if (maxBFSIteration == -1) {
			maxBFSIteration = Integer.parseInt(getContext().getConfiguration()
					.get(NUM_STEP_SIMULATION_END_BFS));
		}
		if (maxTraversalLength == -1) {
			maxTraversalLength = Integer.parseInt(getContext()
					.getConfiguration().get(MAX_TRAVERSAL_LENGTH));
		}
		if (source == "") {
		...
		if (commonReadId == -1) {
			commonReadId = Long.parseLong(getContext().getConfiguration().get(
					COMMOND_READID));
		}
		if (outgoingMsg == null)
	...
	public void BFSearch(Iterator<BFSTraverseMessage> msgIterator,
		...
		HashMapWritable<LongWritable, BooleanWritable> unambiguousReadIds = vertex
				.getUnambiguousReadIds();
		...
		while (msgIterator.hasNext()) {
			...
			if (incomingMsg.getTargetVertexId().equals(getVertexId())) {
				...
				HashMapWritable<LongWritable, PathAndEdgeTypeList> pathMap = vertex
						.getPathMap();
				if (pathMap.containsKey(commonReadId)) { // if it's ambiguous
					...
					unambiguousReadIds.put(new LongWritable(commonReadId),
							new BooleanWritable(false));
					continue; // stop BFS search here
				} else { // if it's unambiguous path, save
					VKmerList updatedKmerList = new VKmerList(
							incomingMsg.getPathList());
					updatedKmerList.append(getVertexId());
					// doesn't need to update edgeTypeList
					PathAndEdgeTypeList pathAndEdgeTypeList = new PathAndEdgeTypeList(
							updatedKmerList, incomingMsg.getEdgeTypeList());
					pathMap.put(new LongWritable(commonReadId),
							pathAndEdgeTypeList);
					unambiguousReadIds.put(new LongWritable(commonReadId),
			...
			int totalBFSLength = updateBFSLength(
					incomingMsg,
	...
	public void compute(Iterator<BFSTraverseMessage> msgIterator) {
		...
		if (getSuperstep() == 1) {
			...
			SearchInfo srcSearchInfo = new SearchInfo(srcNode,
					READHEAD_ORIENTATION.UNFLIPPED);
			SearchInfo destSearchInfo = new SearchInfo(destNode,
					READHEAD_ORIENTATION.FLIPPED);
