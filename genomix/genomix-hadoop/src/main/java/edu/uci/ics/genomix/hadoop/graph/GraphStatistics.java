...
public class GraphStatistics extends MapReduceBase implements
    ...
    public void map(VKmerBytesWritable key, NodeWritable value, OutputCollector<Text, LongWritable> output,
        ...
        reporter.getCounter("startRead-bins", Integer.toString(Math.round(value.getStartReads().getCountOfPosition()))).increment(1);
        reporter.getCounter("totals", "startRead").increment(Math.round(value.getStartReads().getCountOfPosition()));
        
        reporter.getCounter("endRead-bins", Integer.toString(Math.round(value.getEndReads().getCountOfPosition()))).increment(1);
    ...
    public void run(String inputPath, String outputPath, GenomixJobConf baseConf) throws IOException {
