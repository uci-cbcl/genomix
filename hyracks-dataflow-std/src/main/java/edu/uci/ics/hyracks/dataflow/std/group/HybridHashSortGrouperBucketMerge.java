...
import edu.uci.ics.hyracks.dataflow.std.util.ReferenceHashEntry;
import edu.uci.ics.hyracks.dataflow.std.util.ReferencedBucketBasedPriorityQueue;

public class HybridHashSortGrouperBucketMerge {
    private static final Logger LOGGER = Logger.getLogger(HybridHashSortGrouperBucketMerge.class.getName());
    ...
    int ioCounter = 0;
    int mergeLevels = 0;
    long comparisonCount = 0, readTime = 0, writeTime = 0;
    ...
    public void initialize(LinkedList<RunFileReader> runFiles) throws HyracksDataException {
        ...
        try {
            if (runs.size() <= 0) {
                ...
                while (runs.size() > 0) {
                    try {
                        ...
                        LOGGER.warning("HybridHashSortBucketMerge-DoPass-" + mergeLevels + "\t"
            ...
            mergeState.close();
            LOGGER.warning("HybridHashSortGrouperBucketMerge\t" + (System.currentTimeMillis() - mergeTimer) + "\t"
                    + ioCounter + "\t" + mergeLevels + "\t" + comparisonCount + "\t" + readTime + "\t" + writeTime);
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < runNumber; i++) {
                ...
                for (int j = 0; j < runFrameLimit; j++) {
                    ...
                    readTime += System.currentTimeMillis() - timer;
                    if (hasNextFrame) {
    ...
    private void flushOutFrame(IFrameWriter writer, boolean isFinal) throws HyracksDataException {
        ...
        for (int i = 0; i < outFrameAccessor.getTupleCount(); i++) {
            ...
            if (!writerAppender.appendSkipEmptyField(finalTupleBuilder.getFieldEndOffsets(),
                ...
                if (!isFinal)
                    writeTime += System.currentTimeMillis() - timer;
